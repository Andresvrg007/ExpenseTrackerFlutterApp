...
class PDFReportService {
  ...
  static Future<Map<String, dynamic>> generateMonthlyPDFReport({
    ...
    try {
      ...
      final filteredTransactions = results[0] as List<TransactionModel.Transaction>;
      ...
      final savedFileInfo = await _savePDFInMainThread(finalPdf);
      ...
      return {
        ...
        'threadsUsed': 4,
        'message': 'PDF generated successfully using $processingTime ms with 4 parallel threads'
      };
  ...
  /// HILO 1: Filtrar transacciones del mes actual
  static Future<List<TransactionModel.Transaction>> _filterCurrentMonthTransactions(
  ...
  static Future<Map<String, dynamic>> _calculateStatistics(
    ...
    return await Isolate.run(() async {
      ...
      final monthTransactions = allTransactions.where((t) => 
        t.fecha.month == currentMonth && t.fecha.year == currentYear
      ).toList();
      ...
      final averageTransaction = monthTransactions.fold(0.0, (sum, t) => sum + t.amount) / monthTransactions.length;
  ...
  /// HILO 3: Generar datos básicos del PDF (sin crear archivo)
  static Future<Map<String, dynamic>> _generatePDFBasicContent(
    ...
    return await Isolate.run(() async {
      ...
      // Filtrar transacciones del mes actual
      final monthTransactions = allTransactions.where((t) => 
        t.fecha.month == now.month && t.fecha.year == now.year
      ).toList();

      // Simular trabajo intensivo de procesamiento
      for (int i = 0; i < 1500000; i++) {
        final dummy = i.toString() + 'processing';
      }
      ...
      final contentData = {
        ...
        'limitedTransactions': monthTransactions.take(50).map((t) => {
  ...
  /// HILO 4: Procesar validaciones y datos adicionales
            ...
            pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  ...
                  _buildPDFSummaryRow('Total Income', statistics['totalIncome'], PdfColors.green700),
                  pw.SizedBox(height: 8),
                  _buildPDFSummaryRow('Total Expenses', statistics['totalExpenses'], PdfColors.red700),
                  pw.SizedBox(height: 12),
                  pw.Divider(color: PdfColors.grey400),
                  pw.SizedBox(height: 12),
                  _buildPDFSummaryRow('Net Balance', statistics['netBalance'], 
                    statistics['netBalance'] >= 0 ? PdfColors.blue700 : PdfColors.orange700),
                ],
              ),
            ),
            ...
            pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  ...
                  pw.SizedBox(height: 8),
                  pw.Text('✓ Thread 1: Filtered ${filteredTransactions.length} transactions'),
                  pw.Text('✓ Thread 2: Calculated ${statistics['totalTransactions']} statistics'),
                  ...
                  child: pw.Row(
                    ...
                    children: [
                      ...
                      pw.Text(
                        ...
                        style: pw.TextStyle(
                          fontWeight: pw.FontWeight.bold,
                          color: transaction.tipo == 'ingreso' ? PdfColors.green600 : PdfColors.red600,
                        ),
                      ),
                    ],
                  ),
  ...
  static Future<Map<String, dynamic>> _savePDFInMainThread(pw.Document pdf) async {
    print('💾 Saving PDF in main thread...');

    try {
      // ✅ CAMBIAR: Guardar en Downloads en lugar de documents
      final dir = await getExternalStorageDirectory(); // Downloads públicos
      // O usar: final dir = await getApplicationDocumentsDirectory(); // Interno app
      
      // ✅ NUEVA OPCIÓN: Usar Downloads
      ...
      final reportsDir = Directory('${downloadsDir!.path}/FinanceReports');
      ...
      print('📍 Location: ${file.path}');  // ✅ AGREGAR LOG DE UBICACIÓN
  ...
  // Helper methods
  static pw.Widget _buildPDFSummaryRow(String label, double amount, PdfColor color) {
    return pw.Container(
      padding: pw.EdgeInsets.symmetric(vertical: 8),
      decoration: pw.BoxDecoration(
        color: PdfColors.grey100,  // ✅ CAMBIAR: usar color fijo en lugar de withOpacity
        borderRadius: pw.BorderRadius.circular(4),
      ),
      child: pw.Row(
        mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
  ...
  static int _findDuplicateTransactions(List<TransactionModel.Transaction> transactions) {
    final seen = <String>{};
    int duplicates = 0;
  ...
  static double _calculateDataIntegrity(List<TransactionModel.Transaction> transactions) {
  ...
  static Map<String, double> _calculateMonthlyTrends(List<TransactionModel.Transaction> transactions, DateTime now) {
    final trends = <String, double>{};
    for (int i = 0; i < 6; i++) {
      final targetDate = DateTime(now.year, now.month - i, 1);
      final monthKey = DateFormat('MMM yyyy').format(targetDate);
      final monthTotal = transactions
          .where((t) => t.fecha.year == targetDate.year && t.fecha.month == targetDate.month)
          .fold(0.0, (sum, t) => sum + t.amount);
